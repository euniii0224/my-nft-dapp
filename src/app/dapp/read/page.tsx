...
export default function ReadPage() {
  ...
  const balanceOfRead = useReadContract({
    ...
    args: [(balanceOwner.trim() || address || ZERO_ADDRESS) as `0x${string}`],
    query: { enabled: true },
  })

  const [tokenIdQuery, setTokenIdQuery] = useState<string>('0')
  const tokenIdBigInt = useMemo(
    () => asBigInt(tokenIdQuery) ?? BigInt(0),
    [tokenIdQuery],
  )
      ...
      <Card title="balanceOf (읽기)">
        <div className="flex flex-col gap-3">
          ...
          <div className="rounded-xl bg-zinc-50 p-3 text-sm dark:bg-black/40">
            ...
            <div className="mt-1 font-semibold">
              {balanceOfRead.isError
                ? '에러'
                : String(balanceOfRead.data ?? '—')}
            </div>
          </div>
        </div>
      </Card>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            ...
            <div className="rounded-xl bg-zinc-50 p-3 text-sm dark:bg-black/40">
              ...
              <div className="mt-1 break-all font-mono text-xs">
                {tokenURIRead.isError
                  ? '에러/존재X'
                  : String(tokenURIRead.data ?? '—')}
              </div>
            </div>
            <div className="rounded-xl bg-zinc-50 p-3 text-sm dark:bg-black/40">
              <div className="text-xs text-zinc-600 dark:text-zinc-400">
